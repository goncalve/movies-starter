...
watchEffect(() =>  {
  ...
  if(query.value){
    /* regex basé sur query.value sans tenir
    compte la casse avec le flag 'i' */
    let regex = new RegExp(query.value, 'i')

    console.log(regex) /* a remplacer */
    /* il faut remplacer le contenu de films.value
    en ne gardant dans ce tableau que les films 
    qui contiennent query.value dans leur titre */
  }
})
</script>
